...
const data = await page.evaluate(() => {
  ...
  document.querySelectorAll('.zw-palace').forEach((pal) => {
    ...
    pal.querySelectorAll('.zw-star-block').forEach((blk) => {
      ...
      blocks.push({
        text: blk.textContent.slice(0, 16),
        cls: Array.from(blk.classList).filter(c => c.startsWith('zw-star-block')).join(' '),
...
data.forEach((p) => {
  console.log(`\n[${p.name}]`);
  p.blocks.forEach((b) => console.log(`  ${b.cls}: ${b.text}: x=${b.x} y=${b.y} w=${b.w} h=${b.h}`));
});
