#!/usr/bin/env python
"""替换 bz_geju / bz_xiyong / bz_congge 三张卡为旧标题+新数据"""
...
# bz_geju 替换块
# ═══════════════════════════════════════════════════════
old_geju = """    // --- bz_geju: 保持旧 (不在替换范围) ---"""

new_geju = r"""    // --- bz_geju: 八字格局 → 旧标题 + 新管道数据 ---
    else if (cid === 'bz_geju') {
      ...
      var yueSS = typeof _localSShen === 'function' ? _localSShen(riGan, _DG_GAN[yueBenqi]||0) : '';
      ...
      var body = '<div class="card card-accent-gold" data-card-id="bz_geju">'
        ...
        + '<div class="bz-dingge-step-body">日主 <span class="bz-dingge-highlight">' + dmGan + '</span>（' + (riGan%2===0?'阳':'阴') + dmWx + '）　'
        ...
        + '<div class="bz-dingge-step-body">月令 <span class="bz-dingge-gold">' + yueZhi2 + '</span>，本气 <span class="bz-dingge-gold">' + yueBenqi + '</span>'
        + '（' + (yueBenqi ? (['甲','乙','丙','丁','戊','己','庚','辛','壬','癸'][_DG_GAN[yueBenqi]||0]||'') + ('阳' : '阴') : '') + '），'
        + '本气十神：<span class="bz-dingge-highlight">' + (yueSS||'') + '</span>。'
        + '</div></div>'
      ...
      for (var pi4 = 0; pi4 < 4; pi4++) {
        var gz4 = GAN[ob['b'+(pi4+1)]%10] + ZHI[ob['b'+(pi4+1)]%12];
        var benqi4 = (typeof _DG_BENQI !== 'undefined') ? _DG_BENQI[ob['b'+(pi4+1)]%12] : '';
        var ss4 = typeof _localSShen === 'function' && benqi4 ? _localSShen(riGan, _DG_GAN[benqi4]||0) : '';
        body += (pi4>0?'　':'') + ['年','月','日','时'][pi4] + '柱 ' + gz4 + ' 本气' + benqi4 + ' → ' + ss4;
      }
      body += '</div></div>'

        // 四、透干验证 → 新管道 zhengge.source
      ...
      var isStrong2 = mdAnalysis.strengthLevel === '极旺' || mdAnalysis.strengthLevel === '偏旺';
      var isWeak2 = mdAnalysis.strengthLevel === '极弱' || mdAnalysis.strengthLevel === '偏弱';
      if (isStrong2) body += '日主偏旺 → 宜<span class="bz-dingge-gold">泄克耗</span>（官杀/食伤/财星）。';
      else if (isWeak2) body += '日主偏弱 → 宜<span class="bz-dingge-gold">生扶</span>（印星/比劫）。';
      ...
      if (zgPat.boundary) body += '<br>⚠ 边界命例：' + (zgPat.boundaryNote||'');
      body += '<br>据《子平真诠》卷五各论，格局成败决定命局层次高下。'
        + '</div></div>'
    ...
    // --- bz_xiyong: 八字喜用 → 旧标题 + 新管道数据 ---
      ...
      if (mdAnalysis.verify && mdAnalysis.verify.passed) {
        body += '验算<span class="bz-dingge-gold">通过</span>（置信度' + (mdAnalysis.verify.confidence||'?') + '%）。<br>'
          + '用神方向与日主力量一致：'
          + (isWeak2?'身弱 → 生扶（印星/比劫）' : isStrong2?'身旺 → 泄克耗（官杀/食伤/财星）' : '中和 → 格局用神')
          + '，自洽无矛盾。';
      } else {
    ...
    // --- bz_congge: 八字从格 → 旧标题 + 五条件检查 + 核心提示 ---
      ...
      var condDetails = [];
      if (congCheck && congCheck.conditions) {
        for (var ci = 0; ci < congCheck.conditions.length; ci++) {
          condDetails.push({
            name: condNames[ci] || '条件'+(ci+1),
            pass: congCheck.conditions[ci],
            detail: congCheck.reasons ? (congCheck.reasons[ci]||'') : ''
          });
        }
      }
      ...
      var body = '<div class="card" data-card-id="bz_congge">'
        ...
        + '月令 <span class="bz-dingge-gold">' + yueZhi4 + '</span>（' + dmWx3 + ('旺相休囚死'.charAt(riGan>>1)) + '）<br>';
      // 根气检测
      var rootCount = 0;
      if (wxCalc && wxCalc.details) {
        for (var ri = 0; ri < wxCalc.details.length; ri++) {
          if (wxCalc.details[ri].wx === dmWx3 && wxCalc.details[ri].breakdown.tonggen) {
            rootCount = wxCalc.details[ri].breakdown.tonggen.items ? wxCalc.details[ri].breakdown.tonggen.items.length : 0;
          }
        }
      }
      body += '日主根气：' + (rootCount > 0 ? '<span class="bz-dingge-gold">有根（通根' + rootCount + '处）</span>' : '<span style="color:var(--color-cinnabar)">无根</span>')
        + '</div></div>'

        // 二、从格五条件逐项检查（替换旧的百分比表）
      ...
      if (condDetails.length > 0) {
        for (var cdi = 0; cdi < condDetails.length; cdi++) {
          body += '<tr><td style="padding:4px 8px;border:1px solid var(--border-ink)">' + condDetails[cdi].name + '</td>'
            + '<td style="padding:4px 8px;border:1px solid var(--border-ink);color:' + (condDetails[cdi].pass?'#5D8A7C':'var(--color-cinnabar)') + ';font-weight:700">'
            + (condDetails[cdi].pass ? '✅ 满足' : '❌ 不满足') + '</td></tr>';
        }
      } else {
        body += '<tr><td colspan="2" style="padding:8px;text-align:center">从格五条件数据未生成（新管道 _congCheck_md 未执行）</td></tr>';
      }
      body += '</table>'
        + '据《子平真诠》卷五·论从格："有印相生，虽弱不从。"<br>'
        + '据《滴天髓》任注："凡从财官食伤，最忌印绶比劫。"'
        + '</div></div>'
      ...
      if (cgPat.isCong) {
        body += '<div class="bz-dingge-step bz-section--red" style="margin-top:12px">'
          ...
          + '从格之命，一生需<span style="color:#C41E0A;font-weight:700">「顺势而不逆」</span>。逢喜神流年大运必发达，逢忌神必遭重挫。<br>'
          + '《渊海子平》云："从格无根，方论从。"大运见印比即破，逢之需格外谨慎。<br>'
          + '《子平真诠》："真从之格局，逢印比运即破。"破格时多应灾祸，事业反复，健康受损。'
          + '</p>'
...
# Now find and replace each block in the file
# Find the position of the three old placeholders and replace them

# Strategy: find the start of bz_geju placeholder and end of bz_congge placeholder,
# replace the entire range with our new blocks

geju_start = content.find("// --- bz_geju: 保持旧 (不在替换范围) ---")
congge_marker = "// 4张纯数据卡"
congge_start = content.find(congge_marker, geju_start)

if geju_start > 0 and congge_start > 0:
    # Find the actual end of the bz_congge block
    # It's between bz_congge placeholder and "// 4张纯数据卡"

    # Replace the section from bz_geju to just before "// 4张纯数据卡"
    content = content[:geju_start] + new_geju + "\n    " + content[congge_start:]
    print("OK: 3 cards replaced")
else:
    print(f"FAIL: markers not found geju={geju_start} congge={congge_start}")
