...
// 找含 神煞/称骨/骨 的字段
for(const k of keys){const s=JSON.stringify(d[k]);if(/神煞|称骨|骨重|骨|煞/.test(k)||/神煞|称骨|骨重/.test(s.slice(0,200))) console.log('FIELD['+k+'] len='+s.length+' sample='+s.slice(0,160));}
console.log('\ntianLuoDiWang:', JSON.stringify(d.tianLuoDiWang).slice(0,200));
